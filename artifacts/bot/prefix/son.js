...
async function execute(client, message, args) {
  ...
  if (!target) {
    const r = await message.reply({ ...v2([container([text('❌ Укажите пользователя: `?son @user`')])]) });
    return setTimeout(() => r.delete().catch(() => {}), 10000);
  }
  if (target.user.bot) {
  ...
  await message.channel.send({
    ...
    components: [
      container([
        text(`### Тебе предложили стать частью (сыном) семейной пары : <@${p1}> + <@${p2}>`),
        separator(),
        actionRow([
          button(`child_accept_${marriage.id}_${target.id}_son`, { label: 'Я согласен', style: 2 }),
          button(`child_reject_${marriage.id}_${target.id}_son`, { label: 'Отклоняю', style: 2 }),
        ]),
      ]),
    ],
  });
